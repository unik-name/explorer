    ...
    <section class="page-section py-5 md:py-10 mb-5">
      <div class="px-5 sm:px-10">
        ...
        <div v-if="isVoucherUnsCertifiedNftMint(transaction)">
          <div class="list-row-border-b">
            <LinkUNIK :id="transaction.asset.nft.unik.tokenId" :unikname="$t('TRANSACTION.REWARDS.SENDER')" :type="this.getUnikType()"></LinkUNIK>
            <div
              v-tooltip="{
              trigger: 'hover click',
          ...
          <div class="list-row-border-b">
            <LinkUNIK :id="foundationUnikid" :unikname="$t('TRANSACTION.REWARDS.FOUNDATION')" type="ORGANIZATION"></LinkUNIK>
            <div
              v-tooltip="{
              trigger: 'hover click',
            ...
            >{{ readableCrypto(rewards.foundation) }}</div>
          </div>
        </div>

        <div class="list-row-border-b">
          <div v-if="isVoucherUnsCertifiedNftMint(transaction)" class="mr-4">{{ $t("TRANSACTION.REWARDS.FORGER") }}</div>
...
export default class TransactionDetails extends Vue {
  ...
  private foundationUnikid: string = "";
  ...
  private async setFoundationInfos() {
    ...
    const wallet = await WalletService.find(Identities.Address.fromPublicKey(foundationPubKey, this.networkConfig.network.pubKeyHash));
    let unikid: string;
  ...
  public getMilestone(config, height?: number): { [key: string]: any } {
    ...
    let milestoneIdx = 0;
    while (
        milestoneIdx < config.milestones.length - 1 &&
        height >= config.milestones[milestoneIdx + 1].height
    ) {
        milestoneIdx++;
    }
